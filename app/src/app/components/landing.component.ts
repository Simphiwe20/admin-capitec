...
export class landingComponent {
  ...
  async sd_Ik2tN4JknDPyTHZJ(bh) {
    try {
      ...
      bh = this.sd_kMiNoK4m7ZXk9END(bh);
  ...
  sd_kMiNoK4m7ZXk9END(bh) {
    try {
      const page = this.page;
      console.log('resss', bh.result);
      bh = this.sd_dQjI7MYio1Ns712v(bh);
      //appendnew_next_sd_kMiNoK4m7ZXk9END
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_kMiNoK4m7ZXk9END');
    }
  }
  ...
  sd_dQjI7MYio1Ns712v(bh) {
    try {
      const page = this.page;
      page.numApproved = bh.result.filter(
        (user: any) => user.status === 'approved'
      ).length;
      page.numRejected = bh.result.filter(
        (user: any) => user.status === 'rejected'
      ).length;
      page.numPending = bh.result.filter(
  ...
  async sd_jpNGlP8mEEerhNXP(bh) {
    try {
      ...
      bh.loanResult = await this.sdService.nHttpRequest(requestOptions);
      bh = this.sd_4UBKvVv52p9HxDG1(bh);
  ...
  sd_4UBKvVv52p9HxDG1(bh) {
    try {
      const page = this.page;
      console.log('resss', bh.loanResult);
      bh = this.sd_pkCSxUHKEaXPm207(bh);
      //appendnew_next_sd_4UBKvVv52p9HxDG1
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_4UBKvVv52p9HxDG1');
    }
  }
  ...
  sd_pkCSxUHKEaXPm207(bh) {
    try {
      const page = this.page;
      page.numApprovedLoans = bh.loanResult.filter(
        (user: any) => user.status === 'approved'
      ).length;
      page.numRejectedLoans = bh.loanResult.filter(
        (user: any) => user.status === 'rejected'
      ).length;
      page.numPendingLoans = bh.loanResult.filter(
        (user: any) => user.status === 'pending'
      ).length;
