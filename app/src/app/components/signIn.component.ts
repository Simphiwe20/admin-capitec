...
export class signInComponent {
  ...
  async sd_gNR1b9c1Ugt2LbrH(bh) {
    try {
      if (
        ...
        bh = this.sd_dS55rP0WEV9J8vI6(bh);
      } else {
        bh = await this.sd_wEsVzgdVR8TckF8Z(bh);
      }
  ...
  sd_dS55rP0WEV9J8vI6(bh) {
    try {
      const page = this.page;
      console.log('Form valid');
      bh = this.sd_bfsbF7gCB0nskEOk(bh);
      //appendnew_next_sd_dS55rP0WEV9J8vI6
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_dS55rP0WEV9J8vI6');
    }
  }
  ...
  async sd_bfsbF7gCB0nskEOk(bh) {
    try {
      ...
      bh.result = await this.sdService.nHttpRequest(requestOptions);
      bh = this.sd_ySJNC1pJOzndDoPp(bh);
  ...
  async sd_ySJNC1pJOzndDoPp(bh) {
    try {
      if (
        ...
        bh = this.sd_lD5FosM3F7wiEmC2(bh);
      } else {
        bh = await this.sd_5cyvDjiNdvfoWqZ9(bh);
      }

      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_ySJNC1pJOzndDoPp');
    }
  }

  sd_lD5FosM3F7wiEmC2(bh) {
    try {
      this.__page_injector__.get(MatSnackBar).open('Pasword dnt match', 'Ok', {
        duration: 3000,
        direction: 'ltr',
        horizontalPosition: 'center',
        verticalPosition: 'bottom',
      });
      //appendnew_next_sd_lD5FosM3F7wiEmC2
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_lD5FosM3F7wiEmC2');
    }
  }

  sd_5cyvDjiNdvfoWqZ9(bh) {
    try {
      sessionStorage.setItem('loggedInUser', JSON.stringify(bh.result));
  ...
  sd_wEsVzgdVR8TckF8Z(bh) {
    try {
      const page = this.page;
      console.log('Form not valid');
      //appendnew_next_sd_wEsVzgdVR8TckF8Z
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_wEsVzgdVR8TckF8Z');
    }
  }
